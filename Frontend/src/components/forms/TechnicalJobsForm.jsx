...
import {
  FiUser, FiSend, FiCheck, FiX, FiGlobe, FiCpu,
} from "react-icons/fi";
...
const countries = ["Germany", "UAE", "Saudi Arabia", "Qatar", "Canada", "UK", "USA", "Australia", "Other"];

const TechnicalJobsForm = forwardRef(function TechnicalJobsForm(_, ref) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/service-leads/technical-jobs`, payload);
      if (res.data.success) {
        setStatus({ type: "success", message: res.data.message });
        trackFormSubmission("TECHNICAL_JOBS", payload);
        setFormData({
          fullName: "", email: "", phone: "", city: "", whatsapp: "",
          qualification: "", jobField: "", yearsOfExperience: "", keySkills: "",
          languageCertification: "", countryPreferenceWork: [],
          passportAvailable: "", planToGo: "",
        });
        setStep(1);
      }
    } catch (error) {
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
            <FiCpu size={12} /> Apply Now
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 mb-3">
            Launch Your Technical Career Abroad
          </h2>
          <p className="text-slate-500 max-w-xl mx-auto">
            Share your technical profile and our specialist will match you with the best opportunities within 24 hours.
          </p>
        </div>
        ...
        <div className="max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 sm:p-8">
            ...
            <div className="flex items-center justify-center gap-2 mb-8">
              {["Basic Details", "Tech Profile", "Preferences"].map((label, i) => {
                const s = i + 1;
                return (
            ...
            <form onSubmit={handleSubmit}>
              ...
              {step === 2 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                  <div className="flex items-center gap-2 text-blue-700 font-semibold mb-4">
                    <FiCpu size={16} /><span>Technical Profile</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      <label className={labelClass}>Technical Domain</label>
                      <select name="jobField" value={formData.jobField} onChange={handleChange} className={inputClass}>
                        ...
                        <option value="Automotive & Welding">Automotive & Welding</option>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className={labelClass}>Key Skills / Tools</label>
                      <input type="text" name="keySkills" value={formData.keySkills} onChange={handleChange} className={inputClass} placeholder="e.g. Python, AutoCAD, TIG Welding" />
                    </div>
                  </div>
...
export default TechnicalJobsForm;
