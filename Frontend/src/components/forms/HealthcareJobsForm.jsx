...
import {
  FiUser, FiSend, FiCheck, FiX, FiBriefcase, FiGlobe, FiAward, FiHeart,
} from "react-icons/fi";
...
const countries = ["UAE", "Saudi Arabia", "Germany", "UK", "Qatar", "Kuwait", "Australia", "Other"];

const HealthcareJobsForm = forwardRef(function HealthcareJobsForm(_, ref) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/service-leads/healthcare-jobs`, payload);
      if (res.data.success) {
        setStatus({ type: "success", message: res.data.message });
        trackFormSubmission("HEALTHCARE_JOBS", payload);
        setFormData({
          fullName: "", email: "", phone: "", city: "", whatsapp: "",
          qualification: "", specialization: "", yearsOfExperience: "",
          languageCertification: "", jobField: "Healthcare",
      ...
      <div className="container mx-auto px-4 sm:px-6">
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-blue-100 text-blue-700 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider mb-4">
            <FiHeart size={12} /> Apply Now
          </div>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-slate-800 mb-3">
            Start Your Healthcare Career Abroad
          </h2>
          <p className="text-slate-500 max-w-xl mx-auto">
            Fill in your details and our healthcare recruitment specialist will contact you within 24 hours.
          </p>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 p-6 sm:p-8">
            ...
            <form onSubmit={handleSubmit}>
              ...
              {step === 2 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
                  <div className="flex items-center gap-2 text-blue-700 font-semibold mb-4">
                    <FiAward size={16} /><span>Professional Profile</span>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                    ...
                    <div>
                      ...
                      <select name="specialization" value={formData.specialization} onChange={handleChange} className={inputClass}>
                        <option value="">Select</option>
                        <option value="Nursing">Nursing (General / ICU / OT)</option>
                        <option value="Elderly Care">Elderly Care / Geriatric</option>
                        <option value="Doctor / Specialist">Doctor / Specialist</option>
                        <option value="Allied Health">Allied Health (Lab / Physio)</option>
                        <option value="Hospital Support">Hospital Support Staff</option>
                      </select>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className={labelClass}>Language / License</label>
                      <select name="languageCertification" value={formData.languageCertification} onChange={handleChange} className={inputClass}>
                        ...
                        <option value="DHA/MOH/Prometric">DHA / MOH / Prometric</option>
...
export default HealthcareJobsForm;
