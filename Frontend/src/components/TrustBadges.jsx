    ...
    <section className="py-16 sm:py-24 bg-gradient-to-br from-slate-50 to-white">
      <div className="container mx-auto px-4 sm:px-6">
        ...
        >
          ...
          <div className="h-1.5 w-16 sm:w-20 bg-brand-blue mx-auto rounded-full" />
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-20">
          {certifications.map((cert, index) => (
            <motion.div
              ...
              className="bg-white p-4 sm:p-6 rounded-xl sm:rounded-2xl border-2 border-slate-100 hover:border-brand-blue hover:shadow-lg transition-all group"
            >
              <div className="w-10 h-10 sm:w-14 sm:h-14 bg-blue-50 rounded-lg sm:rounded-xl flex items-center justify-center mb-3 sm:mb-4 group-hover:bg-brand-blue transition-colors">
                <cert.icon className="text-xl sm:text-2xl text-brand-blue group-hover:text-white transition-colors" />
              </div>
              <h3 className="font-bold text-brand-dark mb-1 sm:mb-2 text-sm sm:text-base">
                {cert.title}
              </h3>
        ...
        >
          <h3 className="text-xl sm:text-2xl font-bold text-brand-dark text-center mb-6 sm:mb-8">
            ...
            <div className="grid grid-cols-3 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-6">
              {partners.map((partner, index) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ delay: index * 0.1 }}
                  className="flex items-center justify-center p-2 sm:p-4 rounded-lg sm:rounded-xl bg-slate-50 hover:bg-blue-50 transition-colors group"
                >
                  <div className="text-center">
                    <div className="w-8 h-8 sm:w-12 sm:h-12 mx-auto mb-1 sm:mb-2 bg-white rounded-full flex items-center justify-center shadow-sm group-hover:shadow-md transition-shadow">
                      <span className="text-lg sm:text-2xl">🎓</span>
                    </div>
                    <p className="text-[10px] sm:text-xs font-medium text-slate-700 leading-tight">
                      {partner}
                    </p>
                  </div>
                </motion.div>
          ...
          <p className="text-center text-xs sm:text-sm text-slate-500 mt-4 sm:mt-6">
            Direct partnerships with 200+ universities and institutions
            worldwide
          </p>
        </motion.div>
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="bg-gradient-to-r from-brand-blue to-blue-700 rounded-2xl sm:rounded-3xl p-6 sm:p-12 relative overflow-hidden"
        >
              ...
              {achievements.map((achievement, index) => (
                <div key={index} className="text-center">
                  <div className="w-12 h-12 sm:w-16 sm:h-16 bg-white/20 rounded-xl sm:rounded-2xl flex items-center justify-center mx-auto mb-3 sm:mb-4">
                    <achievement.icon className="text-xl sm:text-3xl text-white" />
                  </div>
                  <div className="text-2xl sm:text-4xl md:text-5xl font-bold text-white mb-1 sm:mb-2">
                    {achievement.number}
                  </div>
                  <div className="text-blue-100 font-medium text-xs sm:text-base">
                    {achievement.label}
                  </div>
                </div>
              ))}
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="mt-8 sm:mt-12 flex flex-wrap justify-center gap-4 sm:gap-8 text-xs sm:text-sm text-slate-600"
        >
